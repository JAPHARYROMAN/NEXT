// Command server is the session-service entrypoint. Mirrors the auth/profile pattern.
package main
...
import (
	...
	"github.com/next-ecosystem/next/services/session-service/internal/api"
...
func run() error {
	...
	sessionSvc := api.NewSessionService(pg)
	...
	sessionv1.RegisterSessionServiceServer(grpcSrv, sessionSvc)
	...
	slog.Info("session-service started", "version", version, "http", cfg.HTTPAddr, "grpc", cfg.GRPCAddr)
