// Command server is the upload-service entrypoint.
package main
...
import (
	...
	"github.com/next-ecosystem/next/services/upload-service/internal/api"
...
type config struct {
	...
	BlobRoot     string `env:"UPLOAD_BLOB_ROOT" envDefault:"/tmp/next-uploads"`
}
...
func run() error {
	...
	blobStore, err := blob.NewStore(cfg.BlobRoot)
	if err != nil {
		return err
	}
	...
	uploadv1.RegisterUploadServiceServer(grpcSrv, api.NewUploadService(pg, blobStore))
